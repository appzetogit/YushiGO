...
const toAdminMedicineRow = (ride) => {
  const requestCode = `MED_${String(ride._id).slice(-12).toUpperCase()}`;
  const status = String(ride.status || '').toLowerCase();
  const liveStatus = String(ride.liveStatus || '').toLowerCase();
  const medicine = ride.deliveryId?.medicine || ride.medicine || {};
  ...
  return {
    ...
    deliveryType: medicine.deliveryType || '',
    hasPrescription: Boolean((medicine.prescriptionUrls || []).length),
  };
};
...
export const listRideRequests = async (query = {}) => {
  return listAdminRides({
    query,
    baseFilter: {
      serviceType: { $nin: ['parcel', 'intercity', 'medicine'] },
    },
    variant: 'ride_requests',
    serializer: toAdminRideRow,
  });
...
export const listMedicineOrders = async (query = {}) => {
  return listAdminRides({
    query,
    baseFilter: {
      serviceType: 'medicine',
    },
    variant: 'medicine_orders',
    serializer: toAdminMedicineRow,
    includeParcel: true,
  });
};
