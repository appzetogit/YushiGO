...
// ponytail: student-safety tracking SMS needs its own DLT-approved template registered with the
// provider (SMS_INDIA_HUB_TRACKING_TEMPLATE_ID); until then this logs instead of sending.
export const sendGuardianTrackingSms = async ({ phone, message }) => {
  const config = getSmsIndiaHubConfig();
  const trackingTemplateId = readValue(
    process.env.SMS_INDIA_HUB_TRACKING_TEMPLATE_ID,
  );
  const normalizedPhone = normalizeIndianPhone(phone);

  if (!/^91\d{10}$/.test(normalizedPhone)) {
    return { mode: 'skipped', message: 'Invalid guardian phone number' };
  }

  if (!trackingTemplateId || !config.senderId || !(config.apiKey || (config.user && config.password))) {
    console.warn('[smsService] Guardian tracking SMS not configured; skipping send', { phone: maskSecret(normalizedPhone) });
    return { mode: 'skipped', message: 'SMS provider not configured for tracking messages' };
  }
  ...
  if (!isSuccessfulProviderResponse(response, responseText)) {
    console.warn('[smsService] Guardian tracking SMS failed', responseText);
    return { mode: 'failed', message: responseText };
  }

  return { mode: 'live', message: 'Tracking link sent' };
};
